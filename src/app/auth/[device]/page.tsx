...
export default function DesktopAuth() {
  ...
  const handleAuthorize = async () => {
    try {
      ...
      const response = await fetch(`/auth/?uid=${uid}&device=${params.device}`, {
        method: 'GET',
        headers: {
      ...
      if (data.status === 'success') {
        ...
        window.location.href = `mathsketch://auth?token=${token}&userData=${encodedUserData}&device=${params.device}&uid=${uid}`;
        
        // Show success message with parameters for potential relaunch
        setTimeout(() => {
          router.push(`/auth/success?token=${token}&userData=${encodedUserData}&device=${params.device}&uid=${uid}`);
        }, 1000);
      } else {
